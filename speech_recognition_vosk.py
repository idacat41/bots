...
class SpeechToTextHandler(commands.Cog):
    def __init__(self, bot):
        ...
        self._stream_data = defaultdict(lambda: _StreamData(stopper=None, recognizer=sr.Recognizer(), buffer=array.array('B')))
        ...
        def audio_callback(data, user):
            ...
            if not self._stream_data[user.id]['stopper']:
                self._stream_data[user.id]['stopper'] = self._stream_data[user.id]['recognizer'].listen_in_background(
                    DiscordSRAudioSource(self._stream_data[user.id]['buffer']), self.background_listener(user), self.phrase_time_limit
                )
