...
intents = discord.Intents.all()  # Create an instance of the Intents class
...
async def on_message(message):
	if message.author == bot.user or message.author.id in configFile["IgnoredUsers"]:
		return    
	"""
...
def stable_diffusion_generate_image(prompt):
	try:
		...
		if "selfie" in prompt:
			...
			return response(prompt + configFile["Appearance"])
		else:
			return response(prompt)
	except requests.exceptions.RequestException as e:
			logging.info("An error occurred during the Stable Diffusion API call:", str(e))
			return None
	    # Add the condition statement to check for specific keywords in the prompt
